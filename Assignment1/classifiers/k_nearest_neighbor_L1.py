...
class KNearestNeighborL1(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                #####################################################################
                # TODO: 
                #计算第i个测试点与第j个训练点之间的l1距离，并将结果存储在dists[i，j]中。
                ...
                diff = np.abs(tmp_test - tmp_train)
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            #######################################################################
            # TODO:                                                               
            #计算第i个测试点与所有训练点之间的l1距离，并将结果存储在dists[i，：]中。
            #不要使用np.linalg.norm()。                                     
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            ...
            diff = np.abs(self.X_train - tmp_test)
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # TODO:                                                                 
        #在不使用任何显式循环的情况下，计算所有测试点和所有训练点之间的l1距离，
        #并将结果存储在dists中。                                                     
        #您应该仅使用基本的数组操作来实现此功能。
        #不可以使用scipy中的函数以及函数np.linalg.norm()。
        #
        #提示：尝试使用矩阵乘法和广播总和来计算l1距离。                           
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****

        # |x-y| = np.sqrt((x-y)^2) = np.sqrt(x^2+y^2-2xy)
