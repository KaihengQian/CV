...
class KNearestNeighborM3(object):
    """ a kNN classifier with Minkowski distance(p=3) """
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                cubed_diff = np.abs(np.power(tmp_test - tmp_train, 3))
                dists[i][j] = np.cbrt(np.sum(cubed_diff))
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            cubed_diff = np.abs(np.power(self.X_train - tmp_test, 3))
            dists[i,:] = np.cbrt(np.sum(cubed_diff, axis=1))
